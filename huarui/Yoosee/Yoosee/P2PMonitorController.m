...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([AppDelegateYoosee sharedDefault].isDoorBellAlarm) {//透传连接
        
        [[P2PClient sharedClient] sendCustomCmdWithId:contactId password:contactPassword cmd:@"IPC1anerfa:connect"];
    }
    
    //过滤当前被监控帐号的推送显示
    [AppDelegateYoosee sharedDefault].monitoredContactId = contactId;
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if([AppDelegateYoosee sharedDefault].isDoorBellAlarm){//门铃推送,点按开关说话
        self.isTalking = YES;
...
#define CONTROLLER_BTN_TAG_LOCK_KEY 14
...
-(void)initComponent{
    ...
    CGRect rect = [AppDelegateYoosee getScreenSize:NO isHorizontal:YES];
    ...
    int btnCount;
    if ([AppDelegateYoosee sharedDefault].mainController.contact.defenceState == DEFENCE_STATE_NO_PERMISSION|| [AppDelegateYoosee sharedDefault].contact.defenceState == DEFENCE_STATE_NO_PERMISSION) {//访客密码
        ...
        for(int i=0;i<btnCount;i++){
			...
			if(i==0) {
				controllerBtn.tag = CONTROLLER_BTN_TAG_LOCK_KEY;
				[controllerBtn setBackgroundImage:[UIImage imageNamed:@"ico_lock_key"] forState:UIControlStateNormal];
			}else if(i==1){//声音开关
            ...
            if(i==2){
                //非本地设备
                NSInteger deviceType1 = [AppDelegateYoosee sharedDefault].contact.contactType;
                //本地设备
                ...
                if (deviceType1 == CONTACT_TYPE_DOORBELL || deviceType2 == CONTACT_TYPE_DOORBELL) {//支持门铃,点按开关说话
                    if([AppDelegateYoosee sharedDefault].isDoorBellAlarm){//门铃推送
	...
	}else if ([AppDelegateYoosee sharedDefault].mainController.contact.contactType == CONTACT_TYPE_DOORBELL) { //智能门铃
		...
		for(int i=0;i<btnCount;i++){
			...
			if(i==0) {
				...
				if ([AppDelegateYoosee sharedDefault].mainController.contact.defenceState == DEFENCE_STATE_ON || [AppDelegateYoosee sharedDefault].contact.defenceState == DEFENCE_STATE_ON) {
					[controllerBtn setBackgroundImage:[UIImage imageNamed:@"ic_ctl_lock_on.png"] forState:UIControlStateNormal];
					self.isDefenceOn = YES;
				}else if([AppDelegateYoosee sharedDefault].mainController.contact.defenceState == DEFENCE_STATE_OFF || [AppDelegateYoosee sharedDefault].contact.defenceState == DEFENCE_STATE_OFF){
...
//        if ([AppDelegateYoosee sharedDefault].isDoorBellAlarm) { //如果是门铃推送
...
-(void)onControllerBtnPress:(id)sender{
    ...
    switch(button.tag){
		case CONTROLLER_BTN_TAG_LOCK_KEY:
		{
			[Connect2CUHelper unlockDoor:self.view];
		}
			break;
