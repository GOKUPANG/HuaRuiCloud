...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [AppDelegateYoosee sharedDefault].isShowingDoorBellAlarm = NO;
    [AppDelegateYoosee sharedDefault].currentPushedContactId = nil;
}

#pragma mark - 15秒后自动退出门铃推送界面
-(void)back{
    self.isshow = NO;
    AppDelegateYoosee * appDdelegate = [AppDelegateYoosee sharedDefault];
...
//        appDdelegate.window.rootViewController = mainController;
		...
		[rootVC presentViewController:mainController animated:NO completion:^{
			
			if(nil!=contact){
				mainController.contactName = contact.contactName;//昵称显示不对
				...
				[AppDelegateYoosee sharedDefault].isInputtingPwdToMonitor = YES;
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    [AppDelegateYoosee sharedDefault].isInputtingPwdToMonitor = NO;
    appDdelegate.lastShowAlarmTimeInterval = [Utils getCurrentTimeInterval];
//    appDdelegate.isShowAlarmTip = NO;
}
