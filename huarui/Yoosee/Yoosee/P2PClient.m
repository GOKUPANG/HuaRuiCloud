...
-(void)setAlarmEmailWithId:(NSString *)contactId password:(NSString *)password email:(NSString *)email bOption:(int)bOption smtpServer:(NSString *)smtpServer smtpPort:(int)smtpPort smtpUser:(NSString *)smtpUser smtpPwd:(NSString *)smtpPwd subject:(NSString *)subject content:(NSString *)content isSupportSMTP:(BOOL)isSupportSMTP{
    ...
    sEmail.bOption = bOption;//参数bOption，0表示系统默认邮箱
    ...
    if(isSupportSMTP){//新设备
        ...
        if (bOption == 1) {//主题内容的UI没有实现，只在此传参测试。
        ...
        //针对旧设备，结构体长度36
        fgP2PSendRemoteMessage(contactId.intValue, password.intValue, mesg_id, &sEmail, 36, NULL, 0, 0);
    }
    mesg_id++;
}
